...
- DRIFT: Strapi up/down at **runtime** (populated list vs error boundary vs empty list), auth session cookies, subscription status, email verification state, `STRIPE_WEBHOOK_SECRET` missing at build time
...
- WALLS: `STRIPE_WEBHOOK_SECRET` must be non-empty for production build (`auth.ts` throws if missing outside dev). Set a placeholder for e2e builds. Strapi may be down at **build** time — `/events` uses `connection()` so the list fetch is deferred; build still succeeds. At **runtime**, Strapi failures surface via route `error.tsx` (not cached as `[]`).
